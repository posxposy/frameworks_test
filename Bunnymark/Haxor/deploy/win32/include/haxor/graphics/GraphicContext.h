...
class HXCPP_CLASS_ATTRIBUTES  GraphicContext_obj : public hx::Object{
	...
	public:
		inline void *operator new( size_t inSize, bool inContainer=true,const char *inName="haxor.graphics.GraphicContext")
...
void __GetFields(Array< ::String> &outFields);
		...
		::String __ToString() const { return HX_HCSTRING("GraphicContext","\x47","\x1c","\x31","\x01"); }
		...
		Dynamic CreateVAO_dyn();

		virtual Void DeleteBuffer( int p_id);
		Dynamic DeleteBuffer_dyn();

		virtual Void DeleteVAO( int p_id);
		Dynamic DeleteVAO_dyn();
		...
		virtual Void VertexAttrib1f( int p_location,Float p_x);
		Dynamic VertexAttrib1f_dyn();

		virtual Void VertexAttrib2f( int p_location,Float p_x,Float p_y);
